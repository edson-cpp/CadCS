...
public class CadContas : Principal.Cadastros{
    public dynamic ListarRegistros(int id = 0, bool imp = true){
        ...
        string consulta = "Select id, descricao, agencia, numero From contas";
    ...
    public void MenuContas(){
        ...
        while(true){
            try{
                Console.WriteLine(util.PadCenter(40, " CADASTRO DE CONTAS ", '='));
                util.MenuCad();
    ...
    public void Cadastrar(){
        ...
        string age = util.CampoStr("Agência ", 10);
        string num = util.CampoStr("Número da Conta ", 20);
        try{
            AcessoBD conn = new AcessoBD();
            conn.ExecutarInsercaoAtualizacaoExclusao($"Insert Into contas(descricao, agencia, numero) Values('{desc}', '{age}', '{num}')");
            Console.WriteLine(new string('-', 40));
    ...
    private void Alterar(){
        ...
        if(reg.Count() != 0){
            ...
            string age = util.CampoStr("Agência ", 10, true, "Agência", reg["Agência"]);
            string num = util.CampoStr("Número da Conta ", 20, true, "Número da Conta", reg["Número da Conta"]);

            try{
                AcessoBD conn = new AcessoBD();
                conn.ExecutarInsercaoAtualizacaoExclusao($"Update contas Set descricao = '{desc}', "
                    + $"agencia = '{age}', numero = '{num}' Where id = {id}");
                Console.WriteLine(new string('-', 40));
    ...
    private void Excluir(){
        ...
        if(reg.Count() != 0){
            try{
                ...
                if(resp == "S"){
                    AcessoBD conn = new AcessoBD();
                    conn.ExecutarInsercaoAtualizacaoExclusao($"Delete From contas Where id = {id}");
